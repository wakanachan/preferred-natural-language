...
describe('Bin Scripts E2E Tests', () => {
  ...
  const mcpBinPath = path.join(binDir, 'pnl-mcp.js');
  ...
  describe('pnl CLI bin script', () => {
    it('should execute and show help when run with --help', async () => {
      ...
      expect(stdout).toMatch(/detect|set|show|list/);
    }, 10000);
  ...
  describe('pnl-mcp MCP server bin script', () => {
    it('should start MCP server and log startup message', (done) => {
      const child = exec(`node "${mcpBinPath}"`, { timeout: 3000 });
    ...
    it('should have correct shebang line', () => {
      const content = readFileSync(mcpBinPath, 'utf-8');
      const firstLine = content.split('\n')[0];
    ...
    it('should have executable permissions', () => {
      // Check if file has execute permissions
      expect(() => {
        accessSync(mcpBinPath, constants.X_OK);
      }).not.toThrow();
    });
  });
});
